...
export default function Withdraw({ pair, onClose, isOpen }: any) {
	...
	const max = (_tokenSymbol: string) => balances[_tokenSymbol] ?
		balances[_tokenSymbol]?.holding + balances[_tokenSymbol]?.pending_short : 0;
					...
					<Box>
						<Flex justify={"space-between"}>
							...
							<Text fontSize={"xs"} mb={1}>
								Available {max(tokenSymbol).toFixed(tickToPrecision(pair.quote_tick))}
							</Text>
						</Flex>
